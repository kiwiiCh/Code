...
Changes: added Refresh All button, entry caps, rSpy cooldown, fixed Players tab iteration crash
]]
...
local RefreshBtn = mkBtn(Header, {
	...
	BackgroundColor3 = Color3.fromRGB(20, 100, 160),
	Text = "⟳",
...
mkCorner(RefreshBtn, UDim.new(0, 5))
...
local function addLog(msg, msgType)
	pcall(function()
		...
		while #consoleEntries > MAX_CONSOLE_ENTRIES do
...
clearBtn.MouseButton1Click:Connect(function()
	for _, c in ipairs(consoleEntries) do
...
local function addScriptEntry(s)
	...
	table.insert(scriptEntries, row)
	while #scriptEntries > MAX_SCRIPT_ENTRIES do
		local old = table.remove(scriptEntries, 1)
		pcall(function() old:Destroy() end)
	end
end
...
local function performScriptScan()
	for _, c in ipairs(scriptEntries) do
		pcall(function() c:Destroy() end)
	end
	scriptEntries = {}
	sSeq = 0
...
scanBtn.MouseButton1Click:Connect(performScriptScan)
...
local function makeCard(player)
	...
	while #playerOrder > MAX_PLAYER_CARDS do
		local oldest = playerOrder[1]
		removeCard(oldest)
	end
end
...
local function addRSpyEntry(remote, name, path, ...)
	...
	pcall(function()
		...
		table.insert(rspyEntries, row)
		while #rspyEntries > MAX_RSPY_ENTRIES do
			local old = table.remove(rspyEntries, 1)
			pcall(function() old:Destroy() end)
		end
	end)
end
...
local function hookRemote(remote)
	...
	pcall(function()
		local conn = remote.OnClientEvent:Connect(function(...)
			addRSpyEntry(remote, remote.Name, remote:GetFullName(), ...)
		end)
...
rspyClear.MouseButton1Click:Connect(function()
	for _, c in ipairs(rspyEntries) do
		pcall(function() c:Destroy() end)
	end
	rspyEntries = {}
	rspySeq = 0
